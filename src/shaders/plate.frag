...
/* COLORMAP FUNCTIONS (Unchanged) */
vec3 hsv2rgb(float h, float s, float v) {
  float c = v * s, h6 = h / 60.0;
  float x = c * (1.0 - abs(mod(h6, 2.0) - 1.0));
  vec3 rgb;
  if      (h6 < 1.0) rgb = vec3(c, x, 0); else if (h6 < 2.0) rgb = vec3(x, c, 0);
  else if (h6 < 3.0) rgb = vec3(0, c, x); else if (h6 < 4.0) rgb = vec3(0, x, c);
  else if (h6 < 5.0) rgb = vec3(x, 0, c); else               rgb = vec3(c, 0, x);
  return rgb + (v - c);
}
...
vec3 coolWarm(float t) { t=clamp(t,0.0,1.0); return mix(vec3(0.23,0.3,0.75), vec3(0.7,0.02,0.15), t); }
